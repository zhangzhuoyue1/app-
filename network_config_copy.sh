...
configure_ubuntu() {
    ...
    # 设置NAT规则（优化版）
    iptables -t nat -A POSTROUTING -s 192.168.31.0/24 ! -d 192.168.31.0/24 -o $IN_INTERFACE -j MASQUERADE
    ...
    # 添加端口转发规则（将所有TCP/UDP流量转发到指定端口）
    iptables -t nat -A PREROUTING -i $IN_INTERFACE -p tcp -j REDIRECT --to-ports $FORWARD_PORT
    iptables -t nat -A PREROUTING -i $IN_INTERFACE -p udp -j REDIRECT --to-ports $FORWARD_PORT
...
configure_emulator() {
    ...
    # 删除现有默认路由并添加新的默认路由
    adb shell "ip route del default via 192.168.31.1 dev wlan0 2>/dev/null"
    adb shell "ip route add default via $UBUNTU_IP dev wlan0:1"
    
    # 为指定APP的所有流量打标记
    adb shell "iptables -t mangle -A OUTPUT -m owner --uid-owner $APP_UID -j MARK --set-mark 100"
    
    # 配置DNAT规则（转发所有TCP/UDP流量到Ubuntu）
    adb shell "iptables -t nat -A OUTPUT -m mark --mark 100 -p tcp -j DNAT --to-destination $UBUNTU_IP"
    adb shell "iptables -t nat -A OUTPUT -m mark --mark 100 -p udp -j DNAT --to-destination $UBUNTU_IP"
    
    # 允许标记的流量通过子接口
    adb shell "iptables -A OUTPUT -m mark --mark 100 -o wlan0:1 -j ACCEPT"
    
    # 优化MASQUERADE规则，仅对通过wlan0:1的流量进行NAT
    adb shell "iptables -t nat -A POSTROUTING -o wlan0:1 -j MASQUERADE"
    
    
    # 验证配置（新增）
    echo -e "${YELLOW}验证安卓模拟器配置...${NC}"
...
show_config() {
    ...
    echo -e ""
    echo -e "${GREEN}现在可以在Ubuntu上使用以下命令捕获所有流量:${NC}"
    echo -e "${YELLOW}sudo tcpdump -i $IN_INTERFACE 'src or dst $SUBINTERFACE_IP' -s 0 -w app_traffic.pcap${NC}"
    echo -e ""
    echo -e "${GREEN}提示: 建议同时启动端口监听工具:${NC}"
    echo -e "${YELLOW}sudo nc -l -p $FORWARD_PORT -v > traffic.log${NC}"
    echo -e ""
