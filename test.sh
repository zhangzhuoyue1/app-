...
# 安卓模拟器流量定向转发配置脚本（HTTP/HTTPS专用端口+全流量转发）
...
DEFAULT_SUBINTERFACE_IP="192.168.31.173"  # Ubuntu子接口IP

# 帮助信息
function show_help {
    echo "使用方法: $0 [选项]"
    echo "配置安卓模拟器将指定APP的HTTP/HTTPS流量转发到Ubuntu指定端口，其他流量通过子接口转发"
    echo ""
    echo "必需参数:"
    echo "  -u, --ubuntu-ip       Ubuntu虚拟机IP地址"
    echo "  -p, --port            HTTP/HTTPS转发端口"
    echo "  -a, --app-uid         目标APP的UID"
    echo ""
    echo "可选参数:"
    echo "  -d, --android-ip      安卓模拟器IP地址 [默认: $DEFAULT_ANDROID_IP]"
    echo "  -s, --subinterface-ip Ubuntu子接口IP地址 [默认: $DEFAULT_SUBINTERFACE_IP]"
    echo "  -h, --help            显示此帮助信息"
    echo "  --cleanup             清理所有配置"
    exit 1
}
...
if [ "$1" = "--cleanup" ]; then
    ...
    export SUBINTERFACE_IP=$DEFAULT_SUBINTERFACE_IP
    ...
    adb shell "ip route del default via $UBUNTU_IP" 2>/dev/null
    
    echo -e "${YELLOW}清理Ubuntu配置...${NC}"
    ifconfig $IN_INTERFACE:1 down 2>/dev/null
...
SUBINTERFACE_IP=${SUBINTERFACE_IP:-$DEFAULT_SUBINTERFACE_IP}
...
configure_ubuntu() {
    ...
    # 创建子接口（使用ifconfig）
    ifconfig $IN_INTERFACE:1 $SUBINTERFACE_IP netmask 255.255.255.0 up
    
    # 检查子接口是否创建成功
    SUBINTERFACE_STATUS=$(ifconfig $IN_INTERFACE:1 2>/dev/null)
    
    if [ -z "$SUBINTERFACE_STATUS" ]; then
        echo -e "${RED}错误: 无法创建子接口 $IN_INTERFACE:1${NC}"
        exit 1
    fi
    
    echo -e "${GREEN}子接口 $IN_INTERFACE:1 创建成功${NC}"
    ...
    # 设置NAT规则（优化MASQUERADE规则，匹配整个子网）
    iptables -t nat -A POSTROUTING -s $SUBINTERFACE_IP/24 ! -d 192.168.31.0/24 -o $IN_INTERFACE -j MASQUERADE
    
    # 设置转发规则
    iptables -A FORWARD -i $IN_INTERFACE -o $IN_INTERFACE:1 -j ACCEPT
    iptables -A FORWARD -i $IN_INTERFACE:1 -o $IN_INTERFACE -m state --state RELATED,ESTABLISHED -j ACCEPT
    iptables -A FORWARD -i $IN_INTERFACE:1 -o $IN_INTERFACE -j ACCEPT  # 新增全转发规则
    
    # 添加HTTP/HTTPS端口转发规则
    iptables -t nat -A PREROUTING -i $IN_INTERFACE -p tcp --dport 80 -j DNAT --to-destination $UBUNTU_IP:$FORWARD_PORT
    iptables -t nat -A PREROUTING -i $IN_INTERFACE -p tcp --dport 443 -j DNAT --to-destination $UBUNTU_IP:$FORWARD_PORT
...
configure_emulator() {
    ...
    if [ "$RT_TABLES_EXISTS" -eq 0 ]; then
        echo -e "${YELLOW}/etc/iproute2/rt_tables不存在，使用替代方法配置路由...${NC}"
    else
        # 标准方法：修改rt_tables文件
    ...
    adb shell "iptables -t nat -A OUTPUT -m mark --mark 100 -p tcp ! --dport 80:443 -j DNAT --to-destination $SUBINTERFACE_IP"
    adb shell "iptables -t nat -A OUTPUT -m mark --mark 100 -p udp -j DNAT --to-destination $SUBINTERFACE_IP"
    
    # 优化MASQUERADE规则
    adb shell "iptables -t nat -A POSTROUTING -o wlan0 -j MASQUERADE"
    
    # 添加路由规则，将标记的流量路由到Ubuntu子接口
    adb shell "ip route add default via $UBUNTU_IP dev wlan0 table 100"
    
    # 验证配置
    echo -e "${YELLOW}安卓模拟器路由表:${NC}"
    adb shell "ip route list"
    echo -e "${YELLOW}安卓模拟器iptables NAT规则:${NC}"
...
show_config() {
    ...
    echo -e "  Ubuntu子接口IP: ${YELLOW}$SUBINTERFACE_IP${NC}"
    echo -e "  HTTP/HTTPS转发端口: ${YELLOW}$FORWARD_PORT${NC}"
    echo -e "  APP UID: ${YELLOW}$APP_UID${NC}"
    echo -e "  安卓模拟器IP: ${YELLOW}$ANDROID_IP${NC}"
    echo -e ""
    echo -e "${GREEN}现在可以在Ubuntu上使用以下命令捕获流量:${NC}"
    echo -e "${YELLOW}sudo tcpdump -i $IN_INTERFACE:1 -s 0 -w all_traffic.pcap${NC}"
    echo -e "${YELLOW}sudo tcpdump -i $IN_INTERFACE -s 0 'tcp and (port 80 or port 443)' -w http_traffic.pcap${NC}"
    echo -e ""
    echo -e "${GREEN}提示: 启动端口监听工具捕获HTTP/HTTPS流量:${NC}"
    echo -e "${YELLOW}sudo nc -l -p $FORWARD_PORT -v > http_traffic.log${NC}"
    echo -e ""
